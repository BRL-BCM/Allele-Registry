...
#define LABEL_STRING_CONVERSION(dLabel) textLabels[static_cast<std::vector<std::string>::size_type>(label::dLabel)] = #dLabel; string2label[#dLabel] = label::dLabel;
	...
	void initTextLabels() {
		...
		LABEL_STRING_CONVERSION(MANEPrefRefSeq)
		LABEL_STRING_CONVERSION(transformations)
		LABEL_STRING_CONVERSION(service)
		LABEL_STRING_CONVERSION(algorithms)
		LABEL_STRING_CONVERSION(circular_supported)
		LABEL_STRING_CONVERSION(subsequence_limit)
		LABEL_STRING_CONVERSION(supported_api_versions)
		LABEL_STRING_CONVERSION(metadata)
		LABEL_STRING_CONVERSION(alias)
		LABEL_STRING_CONVERSION(aliases)
		LABEL_STRING_CONVERSION(naming_authority)
		LABEL_STRING_CONVERSION(trunc512)
	}
