...
{
	if (argc != 4) {
		std::cerr << "Three parameters:  text_file_with_IDs  text_file_with_json_output  hg19|hg38" << std::endl;
		return 1;
	}

	std::string const filenameIds  = argv[1];
	std::string const filenameJson = argv[2];
	std::string const genomeBuild  = argv[3];
	...
	while (true)
	{
		...
		auto pos = lineJson.find("\"MyVariantInfo\":[");
		...
		while (true) {
			std::string const start = "\"" + genomeBuild + "id\":\"";
